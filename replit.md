...
- **Prometheus `/metrics`** (`api-server/src/routes/metrics.ts`): dependency-free text-format exposition. Exposes `ssa_station_accounts_total{state}`, `ssa_station_kits_total`, `ssa_active_period_total_{gib,usd}`, `ssa_last_sync_{started,finished}_seconds`, `ssa_last_sync_success`, `ssa_sync_runs_total{status}`, `ssa_sync_running`, `ssa_process_uptime_seconds`. Open by default; gate behind `Authorization: Bearer <METRICS_TOKEN>` if `METRICS_TOKEN` is set.
