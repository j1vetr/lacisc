...
- **Source detection (KIT prefix routing)**: Satcom KITs match `/^KITP\d/i` (e.g. `KITP00409812`); anything else (Tototheo's `KIT` + serial) is Starlink. The shared `/kits` page (Türkçe "Terminaller") merges `useGetKits` + `useGetStarlinkTerminals` into one sortable list with **Satcom (orange)** / **Tototheo (blue)** badges; client-side filter covers both sources, sort spans both. `/kits/:kitNo` is a thin router that dispatches to `SatcomKitDetail` (per-CDR table + footer total) or `StarlinkDetail` (live telemetry card + cumulative-delta daily chart + `poolPlanMonthlyUsage` table). Dashboard KPIs combine totals from both sources when Starlink is enabled and shows a separate Tototheo health row in the System Health card. Starlink hooks are gated on `starlinkSettings.enabled && hasToken` to avoid wasting Tototheo API budget for Satcom-only tenants.
