...
- **WhatsApp eşik bildirimi (2026-05-09 — Task #27)**: E-postadan tamamen ayrı sistem. `whatsapp_settings` (singleton id=1, `enabled` + AES-GCM encrypted `api_key_encrypted` + `endpoint_url` (wpileti.com host'a allowlist'li, UI read-only) + `test_recipient`; `ops_recipients` ve `global_threshold_gb` sütunları LEGACY — runtime hep null yazar/okumaz, sonraki migration'da DROP), `whatsapp_threshold_rules` (plan-bazlı: `min_plan_gb` zorunlu > 0, `step_gb` decimal GB; eski NULL=catchall satırları runtime tarafından yok sayılır, route 400 döner), `whatsapp_alert_state` (composite PK `[source, credential_id, kit_no, period]` + `last_alert_step_gb`). API: `GET/PATCH /whatsapp/settings`, `POST /whatsapp/test`, `GET/POST /whatsapp/threshold-rules`, `DELETE /whatsapp/threshold-rules/{id}`. Trigger noktaları: Satcom `scraper.ts` persist sonrası `await maybeFireWhatsappAlert(...)`, Starlink `starlink-sync.ts:persistMonthlyTotals` ve Norway `leobridge-sync.ts:persistUsage` promise array'e push + `await Promise.all(alertPromises)` (döngü sonunda). **Üç kaynakta da `await` zorunlu** — `void`/fire-and-forget olursa sync fonksiyonu döndüğünde alert henüz `whatsapp_pending_alert` tablosuna yazılmamış olabilir. Atomic claim + pending INSERT artık **tek `db.transaction`** içinde (bkz. 2026-06-07 gotcha): önce `UPDATE whatsapp_alert_state SET last=crossed WHERE last < crossed`; etkilenen satır 0 ise `INSERT ON CONFLICT DO NOTHING` ile satır kur; claim alındıysa aynı tx'te alıcı başına `whatsapp_pending_alert` satırı yaz. Sonuç: e-postadan bağımsız idempotent — aynı eşik için bir KIT'e bir kere mesaj atılır. **Alıcılar (YALNIZCA müşteri)**: `customer_kit_assignments ⋈ admin_users.phone WHERE role='customer' AND phone IS NOT NULL` — müşteri rolü yalnız kendine atanmış KIT için bildirim alır. Operatör/admin/viewer global broadcast yolu DİSPATCH'TEN KALDIRILDI; ops kullanıcılar şu an WhatsApp eşik bildirimi almaz. **Threshold seçimi**: `planAllowanceGb == null` ise (Satcom her zaman, plan kotası bilinmeyen Starlink/Norway dahil) **DOĞRUDAN** `email_settings.threshold_step_gib` fallback'i kullanılır — catchall kuralı kavramı kaldırıldı. Plan biliniyorsa `min_plan_gb NOT NULL AND <= plan` kayıtlardan en yüksek `min_plan_gb`'li seçilir; eşleşme yoksa yine email fallback. **Plan kuralı eşleşmediğinde fallback**: `email_settings.threshold_step_gib` (legacy ad, replit.md'ye göre artık GB olarak yorumlanır) — WhatsApp ve e-posta tek ortak global eşik adımını paylaşır. **Yeni admin_users.phone alanı** (E.164-without-plus normalize: `+90 (532)…`/`0532…`/`5321234567` → `905321234567`). Test endpoint provider HTTP `status` ve gövdesini (ilk 500 char) hem başarı hem hata yanıtında döner. Üretime uygulamak için: `psql "$DATABASE_URL" -f` ile `ALTER TABLE admin_users ADD COLUMN phone text` + 3 yeni tablo (drizzle-kit `admin_users_username_unique` constraint için interactive prompt veriyor — `pnpm db push` çalıştırmayın, manuel SQL kullanın). Frontend: `/settings/whatsapp` (settings + kural CRUD + test; ops textarea kaldırıldı, fallback eşik e-posta ayarlarından read-only gösterilir), admin-users tablosu/dialog'larına telefon alanı.

- **WhatsApp günlük digest — DB kuyruğu (2026-06-07, eski 2026-05-10 in-memory digest'in yerine)**: wpileti.com gibi unofficial WhatsApp wrapper'ları aynı kontağa kısa sürede çok mesaj atınca anti-spam motoruna takılıyor (provider 200 OK dönse bile mesajlar "Mesaj bekleniyor" pending kalıyor). Eski çözüm her sync turu sonunda in-memory buffer'ı flush ediyordu → gün içinde N tur = N mesaj. **Yeni çözüm: günde TEK gönderim.** Alert'ler kalıcı **`whatsapp_pending_alert`** tablosuna yazılır; `startWhatsappDailyDigest()` boot'ta bir kez (downtime catch-up) + `setInterval(60s)` ile `runDailyDigestIfDue()` çağırır. O da Istanbul saatine göre `hour >= whatsapp_settings.daily_send_hour (default 13) && last_daily_flush_date != bugün` ise pencereyi **atomic claim** edip (`UPDATE ... SET last_daily_flush_date = today WHERE last IS DISTINCT FROM today RETURNING`) `flushPendingDigests()` çalıştırır: pending'i oku → alıcıya göre grupla → 1 KIT tek-KIT / 2+ KIT digest şablonu → `enqueueSend` → satırları sil. **KALDIRILDI**: in-memory `pendingDigest` Map, `flushDigestForReceiver`, `flushAllPendingDigests`, `DIGEST_DEBOUNCE_MS`, ve tur-sonu çağrı yerleri (`scheduler.ts` + `routes/station.ts`). **Şablon korundu**: tek-KIT `buildSingleKitMessage` (header `🟢 Veri Uyarısı | %19` + bar + Aşılan/Kalan + Gemi/Dönem), digest `buildDigestMessages` (`🔔 Veri Uyarısı | N KIT` + Dönem + severity emoji + 8-segment bar + `Eşik · Kalan`; planlı KIT yüzde desc, plansız altta totalGb desc; `MAX_KITS_PER_MESSAGE=20`). **İki dayanıklılık kuralı (architect review)**: (1) `flushPendingDigests` throw ederse `runDailyDigestIfDue` pencere claim'ini `row.last`'a geri alır → bir sonraki dakikalık tick aynı gün tekrar dener (yoksa gün gönderildi işaretli kalıp hiç mesaj gitmez). (2) `maybeFireWhatsappAlert` idempotency claim'i + pending INSERT'i tek `db.transaction`'da yapar → arada crash claim'i kalıcı bırakıp pending'i atlamaz. **Bilinen tradeoff**: pending satırları enqueue **sonrası** silinir → tam o anda crash o günkü mesajları kaybeder (eski mimarinin de kabul ettiği at-most-once); yine de gün-boyu biriken alert artık restart'a karşı DB'de güvende. **Prod migration (db push YOK)**: `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "ALTER TABLE whatsapp_settings ADD COLUMN IF NOT EXISTS daily_send_hour integer NOT NULL DEFAULT 13; ALTER TABLE whatsapp_settings ADD COLUMN IF NOT EXISTS last_daily_flush_date date; CREATE TABLE IF NOT EXISTS whatsapp_pending_alert (id serial PRIMARY KEY, receiver text NOT NULL, source text NOT NULL, kit_no text NOT NULL, period text NOT NULL, total_gb double precision NOT NULL, plan_allowance_gb double precision, ship_name text, crossed_step double precision NOT NULL, created_at timestamp NOT NULL DEFAULT NOW()); CREATE INDEX IF NOT EXISTS whatsapp_pending_alert_receiver_idx ON whatsapp_pending_alert (receiver);"`
...
- **Ship-name enrichment**: `CardDetails.aspx?ICCID=<kitNo>` doğrudan URL ile çalışır — KITP, KIT4, KIT3 tüm formatlar için. Eski "iframe context gerekli" notu geçersiz. `enrichShipNames` ve `enrichCardDetails` artık ratedCdrs grid link tıklaması yapmıyor; her KIT için direkt goto + table-row parse. KIT keşfi de haritadan (Map) yapılıyor — `fetchKitLocations` CDR döngüsünden önce çalışır, tüm KIT formatlarını `station_kits`'e seed eder; `extractKitList` artık kullanılmıyor.
...
- **Manuel paket kotası override (2026-06-10)**: 3 tabloya nullable `manual_plan_gb` kolonu eklendi. Prod migration (güvenli, nullable): `psql "$DATABASE_URL" -c "ALTER TABLE station_kits ADD COLUMN IF NOT EXISTS manual_plan_gb double precision; ALTER TABLE starlink_terminals ADD COLUMN IF NOT EXISTS manual_plan_gb double precision; ALTER TABLE leobridge_terminals ADD COLUMN IF NOT EXISTS manual_plan_gb double precision;"` — `pnpm db push` çalıştırma. Backend: PATCH `/station/kits/:kitNo/manual-plan`, `/starlink/terminals/:kit/manual-plan`, `/leobridge/terminals/:kit/manual-plan` (admin). `planAllowanceGb` effective = `manualPlanGb ?? autoValue`. UI: her 3 detail sayfasında "Manuel Kota" widget.

- **Gemi internet satışı kota düşümü (2026-07-10 — Task #37)**: Yeni tablolar `ship_quota_settings` (singleton) + `ship_quota_deductions` (period+KIT+gemi adı unique); detay/karar gerekçesi `docs/CHANGELOG.md`, mimari `docs/ARCHITECTURE.md` "Ship internet resale quota deduction". Endpoint URL BİLEREK hardcoded (whatsapp allowlist deseniyle aynı — kullanıcı değiştiremez, SSRF riski yok). Prod migration (db push YOK): `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "CREATE TABLE IF NOT EXISTS ship_quota_settings (id integer PRIMARY KEY, enabled boolean NOT NULL DEFAULT false, api_key_encrypted text, last_sync_at timestamp, last_sync_status text, last_error_message text, last_period text, updated_at timestamp NOT NULL DEFAULT NOW()); CREATE TABLE IF NOT EXISTS ship_quota_deductions (id serial PRIMARY KEY, period text NOT NULL, external_ship_name text NOT NULL, external_kit_number text NOT NULL DEFAULT '', api_total_gb double precision NOT NULL, matched_source text, matched_kit_no text, match_method text NOT NULL DEFAULT 'none', manual_source text, manual_kit_no text, manual_gb double precision, is_active boolean NOT NULL DEFAULT true, created_at timestamp NOT NULL DEFAULT NOW(), updated_at timestamp NOT NULL DEFAULT NOW()); CREATE UNIQUE INDEX IF NOT EXISTS ship_quota_deductions_period_kit_ship_idx ON ship_quota_deductions (period, external_kit_number, external_ship_name); CREATE INDEX IF NOT EXISTS ship_quota_deductions_period_idx ON ship_quota_deductions (period);"`

- **KIT kaynak taşıma — "en taze kazanır" (2026-06-01)**: Bir KIT bir kaynaktan diğerine taşınınca (Tototheo→Norway gibi) eski satır kalıp KIT "iki yerde" görünür. Kaynak seçimi artık **en güncel `updated_at` kazanır** (`customer-scope.ts::pickWinner` sort `b.ts - a.ts || SOURCE_PRIO`-tie-break); önceki "öncelik önce" davranışı bayat kaynakta takıyordu. Filo haritası (`records.ts /station/fleet-map`) kaynaklar-arası `Map<kitNo>` dedup yapar (taze `lastSeenAt` kazanır) → tek pin; müşteri scope dedup'tan önce uygulanır. **Manuel temizlik**: `DELETE /api/{starlink,leobridge}/terminals/:kit` (admin) — child'lar credential'a FK olduğu için `*_terminal_daily`+`*_terminal_period_total`+`whatsapp_alert_state`(source+kit)+`*_terminals` **tek transaction**'da, tüm credential'lar genelinde silinir; 404 yoksa. **`kits.tsx` BİLEREK dedup YAPMAZ** — bayat satır görünür kalsın ki admin sil ikonuyla (hover overlay + AlertDialog) elle silebilsin. KIT hâlâ o kaynakta aktifse bir sonraki sync onu geri ekler (silme tek seferlik bir temizliktir, kalıcı engel değil). Şema değişmedi → migration yok.
